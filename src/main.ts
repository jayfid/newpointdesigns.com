...
const mobileCheck = (): boolean => {
  ...
  })(navigator.userAgent || navigator.vendor || legacyOpera);

  return check;
};

const addListeners = (): void => {
  const flipElement = getRequiredElement<HTMLElement>(".flip");
  const cardElement = getRequiredElement<HTMLElement>(`.${cardClass}`);

  if (!mobileCheck()) {
    flipElement.addEventListener(
      "mouseenter",
      (event: MouseEvent) => {
        if (event.target !== flipElement || currentState.isLocked) {
          return;
        }
...
const flickCard = (): void => {
  const cardElement = getRequiredElement<HTMLElement>(`.${cardClass}`);

  window.setTimeout(() => {
    cardElement.classList.add(flickedClass);
  }, 200);

  window.setTimeout(() => {
    cardElement.classList.remove(flickedClass);
  }, 425);
};
